...
public class LieuDAO {
    public List<Lieu> findAll() {
        try (Session s = HibernateUtil.getSessionFactory().openSession()) {
            return s.createQuery("from Lieu l order by l.designation", Lieu.class).list();
        }
    }

    public Lieu findById(String id) {
        try (Session s = HibernateUtil.getSessionFactory().openSession()) {
            return s.get(Lieu.class, id);
        }
    }

    public void save(Lieu e) {
        execute(s -> s.persist(e));
    }

    public void update(Lieu e) {
        execute(s -> s.merge(e));
    }

    public void delete(String id) {
        execute(s -> {
            Lieu e = s.get(Lieu.class, id);
            if (e != null)
                s.remove(e);
        });
    }
