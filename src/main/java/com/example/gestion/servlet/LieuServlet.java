...
public class LieuServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (p == null || p.equals("/")) {
            req.setAttribute("lieux", dao.findAll());
            req.getRequestDispatcher("/WEB-INF/views/lieux.jsp").forward(req, resp);
            return;
        }
        if (p.equals("/new")) {
            req.getRequestDispatcher("/WEB-INF/views/lieu-form.jsp").forward(req, resp);
            return;
        }
        if (p.equals("/edit")) {
            req.setAttribute("lieu", dao.findById(req.getParameter("id")));
            req.getRequestDispatcher("/WEB-INF/views/lieu-form.jsp").forward(req, resp);
            return;
        }
        if (p.equals("/delete")) {
            try {
                ...
                flash(req, "success", "Lieu supprimé.");
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if (old == null || old.isBlank()) {
                dao.save(l);
                flash(req, "success", "Lieu créé.");
            } else {
                if (!old.equals(l.getCodelieu())) {
                    dao.delete(old);
                    dao.save(l);
                } else
                    dao.update(l);
                flash(req, "success", "Lieu modifié.");
            }
        } catch (RuntimeException ex) {
